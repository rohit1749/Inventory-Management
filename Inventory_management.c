...
char msg4[128]="Carton 4 has been used at  ";
...
{//PA0 is onboard push button
	...
	GPIOBtn1.GPIO_PinConfig.GPIO_PinPuPdControl = GPIO_NO_PUPD;//no internal pull up resistor
...
void GPIO_ButtonInit2(void)
{
	...
	GPIOBtn2.GPIO_PinConfig.GPIO_PinPuPdControl = GPIO_PIN_PU;// internal pull up resistor
...
int main(void)
{
    ...
    while(1);
		...
		//wait till button4 is pressed
				while( GPIO_ReadFromInputPin(GPIOC,GPIO_PIN_NO_1) != BTN_PRESSED );
				...
				delay();
								   ...
								   concatenate_string(msg4, time_to_string(&current_time));
								   concatenate_string(msg4, am_pm);
								   concatenate_string(msg4, " ");
							       concatenate_string(msg4, date_to_string(&current_date));
							       concatenate_string(msg4, ", ");
								   concatenate_string(msg4, get_day_of_week(current_date.day));
								   concatenate_string(msg4, "\n\r");

				USART_SendData(&usart2_handle,(uint8_t*)msg4,strlen(msg4));
